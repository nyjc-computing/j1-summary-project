...
class Game:
    ...
    def enter(self, choice: str):
        ...
        if choice in "WASDwasd":
            ...
            item = self.player.inventory[int(choice)]
            if item.get_type() == "HealthPotion" or item.get_type() == "AuraPotion":
                self.player.use_item(int(choice),self.get_player_tile().get_monster())
                self.player.remove_item(int(choice))
                print(f'{item.get_name()} used and removed')
            else:
                if self.get_player_tile().get_monster() is not None:
                    self.player.use_item(int(choice),self.get_player_tile().get_monster())
                    print(f'{item.get_name()} used')
                else:
    ...
    def get_player_inventory(self):
        ...
        for item in inventory.items():
            name_inventory[num] = item[1].get_name()
            num += 1
